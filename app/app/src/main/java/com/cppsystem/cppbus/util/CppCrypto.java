...
public class CppCrypto {
    ...
    public static String encrypt(String key, String iv, String data) {
        try {
            if (key.length() < CppCrypto.CIPHER_KEY_LEN) {
                int numPad = CppCrypto.CIPHER_KEY_LEN - key.length();

                for(int i = 0; i < numPad; i++){
                    key += "0"; //0 pad to len 16 bytes
                }

            } else if (key.length() > CppCrypto.CIPHER_KEY_LEN) {
            ...
            Cipher cipher = Cipher.getInstance(CppCrypto.CIPHER_NAME);
    ...
    public static String decrypt(String key, String data) {
        try {
            ...
            ex.printStackTrace();
            Log.e("Error decrypt",ex.getCause()+" "+ex.getMessage());
        }
