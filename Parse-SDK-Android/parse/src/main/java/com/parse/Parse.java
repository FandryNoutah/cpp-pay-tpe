...
import static com.parse.EventuallyPin.PIN_NAME;
...
public class Parse {
    ...
    public static int getOfflineSync(){
        //ArrayList<String> offlineList = EventuallyPin.findAllPinned(new ArrayList<String> ());
        int count=0;
        ...
        try {
            List<EventuallyPin>  eventuallyPinList = query.find();
            for (EventuallyPin event:eventuallyPinList) {
                ParseObject obj = event.getObject();
                if(obj.getClassName().equals("CppTransportEvent"))
                    count++;
    ...
    public static List<ParseObject> getAllOfflineSync(){
        ParseQuery<EventuallyPin> query = new ParseQuery<>(EventuallyPin.class)
                .fromPin(PIN_NAME)
                .ignoreACLs()
    ...
    public static int getCountOfflineSync(){
        //ArrayList<String> offlineList = EventuallyPin.findAllPinned(new ArrayList<String> ());
        int count=0;
        ParseQuery<EventuallyPin> query = new ParseQuery<>(EventuallyPin.class)
                .fromPin(PIN_NAME)
    ...
    public static List<ParseObject> getAllDataOfflineSync(){
        ParseQuery<EventuallyPin> query = new ParseQuery<>(EventuallyPin.class)
                .fromPin(PIN_NAME)
                .ignoreACLs()
                .orderByDescending("time");
    ...
    static void initialize(Configuration configuration, ParsePlugins parsePlugins) {
        ...
        try {
            ...
            Log.e("Server url",new URL(configuration.server).toExternalForm());
